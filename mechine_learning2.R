...
vali_data1= read.csv('H:/prna_transcript_analysis/valid_plasma/pas_output.txt',sep = '\t',skip = 1)
vali_data2= read.csv('H:/prna_transcript_analysis/valid_plasma/ps_output.txt',sep = '\t',skip = 1)
...
for(i in 1:30) seeds[[i]]<- sample.int(n=10000, 1433) #
...
rfeControl <- rfeControl(functions=rfFuncs, seeds = seeds,
                         ...
                         number=10)
rfebreaks <- seq(2, ncol(newdata2), 1)## step size 2 to gradient select variables
...
caret::featurePlot(newdata2[,rfeImp$feature], newdata2$group, plot="box")
...
logcpm_table2$group = rep(c("cancer","normal"),times = c(30,10))
...
newdata3 <- newdata2 %>% dplyr::select(c("group",rfeImp$feature))
newdata3_preProcess <- preProcess(newdata3, method = c("center", "scale"))
Transformed <- predict(newdata3_preProcess, newdata3)
