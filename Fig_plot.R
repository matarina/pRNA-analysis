...
library(yyplot)
#fig1A
df <- data.frame(group=c("all TSS","most representative TSS"),
                 len=c(29598,16455))
df$group <- factor(df$group, levels=c("all TSS", "most representative TSS"))
...
ggsave(filename = 'plot.png',plot = p,width = 11,height = 16)

#fig1B
df <- data.frame(group=c("all TSS","most upstream TSS"),
                 len=c(29598,16455))
df$group <- factor(df$group, levels=c("all TSS", "most upstream TSS"))
fig1b <-ggplot(data=df, aes(x=group, y=len,fill=group)) +
  geom_bar(stat="identity")+
  scale_fill_manual(values = c("#153462","#7F5283"))+
  geom_text(aes(label = len), size = 2.5,color="white",vjust= 1.81, position = "stack")+
  labs(x = "TSS")+labs(y = "Numbers")+theme_classic() +
  theme(panel.background = element_blank(),
        ...
        plot.margin=unit(c(1, 1, 1, 1),'cm'))  
fig1b
ggsave(filename = 'plot2.png',plot = p,width = 11,height = 16)

#fig1C
df2 <- data.frame(group=c("all TSS","psRNA","pasRNA"),
                 len=c(16455,15658,14503))
df2$group <- factor(df2$group, levels=c("all TSS", "psRNA", "pasRNA"))
fig1c <-ggplot(data=df2, aes(x=group, y=len,fill=group)) +
  geom_bar(stat="identity")+
 scale_fill_manual(values = c("#153462","#AA8B56","#CC3636"))+
   geom_text(aes(label = len), size = 2.5,color="white",vjust= 1.81, position = "stack")+
  labs(x = "TSS")+labs(y = "Numbers")+theme_classic() +
  theme(panel.background = element_blank(),
        legend.text = element_text(size = 7),
...
fig1b_c = ggarrange(fig1b,fig1c,align = 'v')
ggsave(filename = 'H:/fig_project/fig1b_c.pdf',plot = fig1b_c,width = 8.27,height =3.69,units = 'in')
...
ggsave(filename = 'H:/fig_project/fig2a_b.pdf',plot = fig2a_b,width = 8.27,height =5.69,units = 'in')
...
library(tidyr)
library(dplyr)
sense = read.csv("H:/prna_transcript_analysis/PCA/ps_output.txt",sep = '\t',row.names = 1)
...
fig2c = ggplot(pca, aes(x=PC1, y=PC2,color=group))+ geom_point(size=3,stroke = 2, alpha = 0.51)+ stat_ellipse(level = 0.999)+
...
ggsave(filename = 'H:/fig_project/fig2c.pdf',plot = fig2c,width = 7.135,height =5.69,units = 'in')
...
pca = plotPCA(vsd, intgroup=c("cell"),returnData = TRUE)
mypal <- c('#219F94','#C65D7B','#325288','#C3B091','#FC997C')
fig2d = ggplot(pca, aes(x=PC1, y=PC2,color=group))+ geom_point(size=3,stroke = 2, alpha = 0.51)+ stat_ellipse(level = 0.999)+
    scale_color_manual(values= mypal)+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),panel.background = element_blank(),
        ...
        axis.title.y = element_text(size = 13))  
fig2d
ggsave(filename = 'H:/fig_project/fig2d.pdf',plot = fig2d,width = 7.135,height =5.69,units = 'in')
...
#loadfonts() 
venn.diagram(
  x,filename = 'fig_project/fig2e.png',
  category.names = c("stem_cell" , "foreskin_fiber " , "prostate", "lymphoblasts","breast"),
  # Circles
  lwd = 2,
  lty = 'blank',
  fill = c("#bc4639", "#E69F00", "#d4bbd8", "#086e7d","#51a16a"),
  # Numbers
  cex = 1.7,
  #fontface = "arial",
...
#loadfonts() 
venn.diagram(
  x,filename = 'fig_project/fig2f.png',
  category.names = c("stem_cell" , "foreskin_fiber " , "prostate", "lymphoblasts","breast"),
  # Circles
  lwd = 2,
  lty = 'blank',
...
ps_mrna = ggplot(b, aes(x=mrna_mean, y=ps_mean,color = group)) + geom_point(size = 2)+ theme_classic()+
...
pas_mrna = ggplot(b, aes(x=mrna_mean, y=pas_mean,color = group)) + geom_point(size = 2)+ theme_classic()+
...
corr
ggsave(filename = 'H:/fig_project/corr.pdf',plot = corr,width = 8.27,height =2.69,units = 'in')
...
without_bp13$group = rep("untarget \nmean :239.83min",nrow(without_bp13))
bp13_all = bind_rows(bp13,without_bp13)
ks.test(bp13$halflife,without_bp13$halflife, alternative = "l")

ps_bp = ggplot(bp13_all, aes( x=halflife,color = group))+stat_ecdf(size=2)+
  annotate("text", x = 300.05, y = 0.073,size = 4, label = "KS-test P-value:2.109e-05")+
...
bp13$halflife = log(2)/bp13$alpha_hepg2
mean(bp13$halflife)
bp13$group = rep("target \nmean :308.78min",nrow(bp13))
without_bp13 = decaydata[!(decaydata$X %in% bp3target$V4),]###删除在另一个表中含有同名的行
without_bp13$halflife = log(2)/without_bp13$alpha_hepg2
mean(without_bp13$halflife)
without_bp13$group = rep("untarget \nmean :242.24min",nrow(without_bp13))
bp13_all = bind_rows(bp13,without_bp13)
ks.test(bp13$halflife,without_bp13$halflife, alternative = "l")

pas_bp = ggplot(bp13_all, aes( x=halflife,color = group))+stat_ecdf(size=2)+
  annotate("text", x = 300.05, y = 0.073,size = 4, label = "KS-test P-value:6.131e-12")+
  scale_color_manual(values = c("#cf3550","#048e9c"))+
  scale_x_continuous(expand = c(0, 5),limits = c(0,495))+
  scale_y_continuous(expand = c(0, 0))+
  labs(y = "Cumulative fraction")+labs(x = " halflife")+
...
prna_bp
ggsave(filename = 'H:/fig_project/fig3d_e.pdf',plot = prna_bp,width = 8.27,height =3.69,units = 'in')
##fig3c
...
library(ggplot2)
prna_mrna = ggplot(mrna_vs, aes( x=min,color = group))+stat_ecdf(size=2)+
...
ggsave(filename = 'H:/fig_project/fig3c.pdf',plot = prna_mrna,width = 4.27,height =3.69,units = 'in')
ggsave(filename = 'H:/fig_project/fig3d_e.pdf',plot = prna_decay,width = 8.27,height =3.69,units = 'in')



halflife1 = data.frame(a = c('a','b','c'),value = c(530.61,252.20,241.34))
halflife1 = ggplot(halflife1,aes(x = a,y = value ,fill = a))+geom_bar(stat="identity")+scale_fill_manual(values = c("#13274f","#ce1141","#e7a801"))+theme_void()
halflife2 = data.frame(a = c('a','b'),value = c(312.79,239.83))
halflife2 = ggplot(halflife2,aes(x = a,y = value ,fill = a))+geom_bar(stat="identity")+scale_fill_manual(values = c("#cf3550","#048e9c"))+theme_void()
halflife3 = data.frame(a = c('a','b'),value = c(308.78,242.24))
halflife3 = ggplot(halflife3,aes(x = a,y = value ,fill = a))+geom_bar(stat="identity")+scale_fill_manual(values = c("#cf3550","#048e9c"))+theme_void()
halflife = ggarrange(halflife1,halflife2,halflife3,ncol = 3)
halflife
ggsave(filename = 'H:/fig_project/fig3cde_halflife.pdf',plot = halflife,width = 8.27,height =3.69,units = 'in')
