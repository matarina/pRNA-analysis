##omic mapped hcc tissue statistic
rm(list=ls())
library(tidyverse)
library(tibble)
clinical  = read.csv('H:/prna_transcript_analysis/hcc_tissue/mongolia/omic_pipe/mongolia_meta.csv',sep = ',')
ps_output = read.csv('H:/prna_transcript_analysis/hcc_tissue/mongolia/omic_pipe/mongolia_ps.csv',sep = ',')
ps_output$geneid <- gsub('^', 'ps_', ps_output$geneid)
pas_output = read.csv('H:/prna_transcript_analysis/hcc_tissue/mongolia/omic_pipe/mongolia_pas.csv',sep = ',')
...
expr = rbind(ps_output,pas_output)

rownames(expr) = expr$geneid
expr = expr[,-1]
advanced_tissue = filter(clinical,stage == 1 & c4 == 'tumor')
early_tissue = filter(clinical,stage == 0 & c4 == 'tumor')

cancer = expr[,colnames(expr) %in% advanced_tissue$Run]
normal = expr[,colnames(expr) %in% early_tissue$Run]
...
colnames(expr_cancer) = rep('Advanced',ncol(expr_cancer))
colnames(expr_normal) = rep('Early',ncol(expr_normal))
expr_prna = column_to_rownames(merge(expr_cancer,expr_normal,by = 'row.names'),var = 'Row.names')
biomarker_expr =expr_prna[c('ps_CBX3_2','ps_LYPD5_2','ps_TRAM2_1','ps_WDR26_3','ps_ACOT2_2','ps_LRRC27_1','ps_IL7_1','ps_ARL6IP6_2','ps_HAAO_2','ps_C8orf41_1'),]
...
library(edgeR)
cpm_expr = data.frame(edgeR::cpm(expr))
...
library(dplyr)
library(tibble)
ps_rawCounts = read.csv('H:/prna_transcript_analysis/hcc_tissue/mongolia/omic_pipe/mongolia_ps.csv',sep = ',')
ps_rawCounts$geneid = gsub('^','ps_',ps_rawCounts$geneid)

pas_rawCounts = read.csv('H:/prna_transcript_analysis/hcc_tissue/mongolia/omic_pipe/mongolia_ps.csv',sep = ',')
...
sampleData <- read.csv("H:/prna_transcript_analysis/hcc_tissue/mongolia/omic_pipe/mongolia_meta.csv",sep = ',')
sampleData = filter(sampleData,c4 == 'tumor')
sampleData = sampleData %>% select('Run','stage') %>% column_to_rownames('Run')
sampleData = na.omit(sampleData)
sampleData$stage <- factor(sampleData$stage)
head(sampleData)
...
library(DESeq2)
deseq2Data <- DESeqDataSetFromMatrix(countData=rawCounts, colData=sampleData, design= ~ stage)
...
EnhancedVolcano(res1,
                ...
                pCutoff = 0.05)
tissue_volcano = EnhancedVolcano(res1,legendPosition = 'none',
                                 gridlines.minor=FALSE, 
                                 gridlines.major=FALSE,lab =rownames(res1),
                                 title = NULL,subtitle = NULL,#legendLabels = NULL,
                                 caption = '',hline = NULL,vline = NULL,
                                 x = 'log2FoldChange',
                                 y = 'padj',parseLabels =TRUE,
                                 border = 'full',
                                 borderWidth = 1,pointSize = 2.0,
                                 axisLabSize = 13,
                                 col=c("#525e75","#86A3B8","#BDCDD6","#ff6361"),
                                 pCutoff = 0.1)

tissue_volcano
ggsave(filename = 'H:/fig_project/tissue_volcano.pdf',plot = tissue_volcano,width = 4.27,height =3.69,units = 'in')
...
##use chatgpt plot a roc curve which auc = 1
